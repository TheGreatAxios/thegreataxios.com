...
preview: "SKALE's native RNG uses threshold signatures from 11/16 consensus nodes to generate free, synchronous, provably random numbers via BLAKE3 hashing, enabling unique NFT generation as demonstrated in Shape Storm's single-ownership roguelike."
---

# The Power of Random

**TL;DR**
SKALE Chains provide native random number generation through threshold signatures from 11/16 asynchronous consensus nodes, creating provably random BLAKE3 hashes each block. Unlike Chainlink VRF, SKALE RNG is free (zero gas fees), synchronous (no callbacks required), and available via pre-compiled contracts. The skale-rng package enables efficient generation of multiple random numbers from a single block signature through bitwise operations and re-hashing. Shape Storm demonstrates this by generating unique NFT attributes with provable randomness, implementing single-ownership mechanics where players can only hold one randomly-generated shape at a time.
...
If your project is in need of blockchain consulting, engineering, or solutions architecture, head over to [https://dirtroad.dev](https://dirtroad.dev/)to inquire.

For game developers and studios, checkout [https://eidolon.gg](https://eidolon.gg/) for support with bespoke game development, security reviews, and consulting.