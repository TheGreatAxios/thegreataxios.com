...
**Complex schemas are handled.** Tools with deep JSON args use dot-path notation — `profile.address.city=Austin` — or a JSON inline fallback:

```
<call>submit_order {"items":[{"id":1,"qty":3}],"shipping":{"address":"..."}}</call>
```

The parser detects which format the model used.

**The accuracy cliff widens.** More real-world tasks sit above the difficulty-5 line than below it. If the compact format continues to handle complex multi-step tasks where JSON Schema fails, the impact on production agent systems is larger than what a 66-call benchmark can capture.
...
- **Strict server-side validation.** Key=value pairs are less strict than JSON Schema. For validated schemas, use the JSON inline fallback — `<call>submit {"key":"value"}</call>` — which the parser handles.
...
- **Deeply nested schemas (5+ levels).** Dot-path flattening gets awkward. The JSON inline fallback handles it, but at that point you are halfway back to native format.
...
- 66 total calls across two Anthropic models (Claude Haiku 4.5 and Claude Sonnet 4.6)
