...
preview: "Implementing a Signing Pool with HD wallet-derived child signers solves EVM nonce collision issues, enabling authoritative servers to scale from a few to hundreds of concurrent blockchain requests per second."
---

# Scaling Authority on the EVM

**TL;DR**
Authoritative servers managing blockchain applications face scaling challenges due to the EVM's sequential nonce requirement per account. A single server account attempting to handle multiple concurrent requests will encounter nonce collisions, causing transaction failures. The solution is implementing a Signing Pool using HD wallets to derive multiple child signers that can handle parallel requests. This design enables scaling from a few requests per second to hundreds, with automatic gas balance checks and dynamic signer selection, making it ideal for high-throughput applications on zero-gas-fee networks like SKALE.
