...
preview: "BITE Protocol implements Proof of Encryption using threshold encryption and BLS signatures to enable MEV-resistant transactions with zero Solidity changes, accessible via cloud APIs for any programming language."
---

# Proof-of-Encryption in the Cloud

**TL;DR**
BITE Protocol implements Proof of Encryption (PoE) using threshold schemes, BLS threshold encryption, and multi-party signatures to enable fully encrypted transactions with MEV resistance. Unlike isolated TEE solutions, PoE embeds encryption directly into consensus through provable mathematics. The protocol requires zero Solidity changes and will be accessible via cloud APIs to enable encrypted transactions across any programming language or environment, with FAIR L1 pioneering the implementation before SKALE Chain adoption.
