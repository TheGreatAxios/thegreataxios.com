...
title: "Programmable Privacy (Part 3/5): Re-Encryption"
description: "Re-encryption enables encrypted data to be forwarded to new recipients without ever decrypting onchain — private information sharing, data delegation, and confidential token transfers."
---

# Programmable Privacy (Part 3/5): Re-Encryption
...
Re-encryption uses threshold cryptography to transform ciphertext encrypted to one key into ciphertext decryptable by another — without ever revealing the plaintext.

Programmable privacy provides two encryption modes:
...
Re-encryption bridges these. Data encrypted via TE can be re-encrypted to any ECIES viewer key. Data encrypted via ECIES can be forwarded to new recipients through threshold operations.
...
The committee holds threshold shares of the private key. No single validator can re-encrypt alone. A supermajority must collaborate, ensuring re-encryption only happens when protocol conditions are satisfied.

For developers, the interface is standard Solidity:

```solidity
// Store threshold-encrypted data
bytes memory encryptedData = BITE.encryptTE(plaintext, publicKey);

// Later: re-encrypt to a specific viewer
BITE.reencryptToViewer(encryptedData, viewerPublicKey);
```
...
Part 5 of this series covers the live poker implementation. Here's how re-encryption works in that context:
...
During the hand, only the player can see their hole cards — decrypted via their private key. At showdown, a CTX decrypts the threshold-encrypted version to evaluate the winner. No single entity ever sees all cards unencrypted until the CTX triggers reveal.
